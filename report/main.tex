...
Ce principe est notamment utilisé pour les déplacements des acteurs. Le déplacement de l'acteur principal est lui engendré par la méthode \textit{Terminal-kit.terminal.grabInput(true)} qui permet la lecture des entrées claviers. Chaque entrée clavier traduit un déplacement via la création d'un nouveau monde introduisant un nouveau poulet ayant reçu un message de déplacement dans sa mailbox.\\ 
...
Afin de modéliser des niveaux de difficulté différentes, nous avons joué sur plusieurs paramètres :
\begin{itemize}
...
Les données du jeu peuvent évoluer à un rythme plus rapide que celui des ticks. Par conséquent, la mise à jour des acteurs doit suivre cette cadence indépendamment du rythme imposé par les ticks. Contrairement au défilement des lignes, déclenché par chaque tick, la mise à jour des acteurs n'entraîne aucun déplacement visuel. Il s'agit uniquement d'un parcours complet de l'ensemble des acteurs pour déclencher leur fonction interne \textit{update}. Cette mise à jour concerne principalement le poulet dont les déplacements doivent suivre les commandes du joueur. La fréquence de la mise à jour influe directement sur la réactivité du jeu et donc sur la précision des mouvements. Elle concerne également le déplacement des boules de feu qui évoluent dans le terrain plus rapidement que le défilement général.\\
...
\item Collisions avec le poulet : elles sont vérifiées par la fonction \textit{checkCollision} qui, en cas de collision avec un acteur, provoque la mort du poulet, comme dit précédemment.
  \item Collisions entre une boule de feu et un acteur : Lorsqu'une collision survient entre une boule de feu et un acteur, le monde actuel est mis à jour afin d'intégrer les effets de cette collision. Tout comme lors du défilement du terrain, ce nouveau monde est placé dans la file des mondes pour actualiser les données. Dans cette version actualisée du monde, la boule de feu impliquée est supprimée et l'acteur atteint est remplacé par un acteur vide, qui ne sera désormais plus affiché.
\end{enumerate}
...
Dans le code, la plupart des fonctions respectent ces principes. On peut prendre comme exemple la génération d'un nouveau monde à chaque modification ou le stockage des acteurs dans des listes de taille fixe. Cependant, la structure du code apporte certaines limites à ce type de programmation. Il existe dans le code des variables dont la modification ne peut pas être traduite autrement qu'avec un effet de bord. Le stockage d'informations générale du jeu est plus difficilement implémentable de manière pure. Il faut alors penser ces variables comme des arguments d'une structure qui représente tout le jeu.\\
On retrouve notamment la file des mondes du jeu qui est modifiée par effet de bord au moment du défilement du terrain. De plus, cette file ne peut pas être traitée comme une constante car elle est intégralement recréée dans certains cas particuliers.
...
fonctions s'éxecutent avec des entrées erronées ou obsoltères. Cela justifie les fréquences des différentes boucles de jeu illustrées dans la figure \ref{fig:acc_tab}. 
